...
> [!WARNING]
> **複数アプリのログイン通知を1本へ集約する目的でこのエンドポイントを使わないでください。**
> Supabase プロジェクトは複数アプリで共有しており、`auth.users` / `auth.sessions` は
> プロジェクトに1つしかありません。そこへ掛けた Database Webhook は**他アプリの
> ログインでも発火**し、`{app_id}` は設定時に選んだ表示名にすぎないため、集約先では
> 全アプリのログインが同じ送信元として並びます。区別を残したいアプリは、アプリ側の
> コードから `source` を付けて共通チャンネルへ送ってください。
