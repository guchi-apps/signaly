...
このモジュールは Signaly 自身が送る側になる経路——`login_notify.py`（Signaly への
ログイン）——で使う。**受信側では使わない。** 受け取った通知を整え直すことはせず、
届いたものはそのまま保存する（整形の定義が送信側と受信側の2か所に散るのを避ける）。
唯一の例外は `field_value()` で、これは受信側（`login_origin.py`）が
フィールド名で値を引くための読み取り用ヘルパー。
